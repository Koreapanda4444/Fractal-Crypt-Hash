...
#ifdef FCH_ENABLE_REDUCED_ROUND_TESTS
#define FCH_REDUCED_ROUND_LINKAGE
#else
#define FCH_REDUCED_ROUND_LINKAGE static
#endif

FCH_REDUCED_ROUND_LINKAGE int fch_mix_compress_rounds(
    uint64_t *state,
    size_t state_words,
    const uint8_t block[FCH_MIX_BLOCK_SIZE],
    size_t block_length,
    ...
    if (state_words != 8u)
        return 0;
    if (rounds == 0u || rounds > FCH_MIX_ROUNDS)
        return 0;
...
#undef FCH_REDUCED_ROUND_LINKAGE
